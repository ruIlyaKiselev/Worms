...
namespace TestProject1.Database
{
    public class PostgresDatabaseTest
    {
        ...
        public void BuddingToOtherWormPosition()
        {
            ...
            try
            {
                postgresDatabase.SaveWorldBehavior(worldBehavior1.Name,
                    Converters.convertListToJson(worldBehavior1.FoodCoords));
                postgresDatabase.SaveWorldBehavior(worldBehavior2.Name,
                    Converters.convertListToJson(worldBehavior2.FoodCoords));
                postgresDatabase.SaveWorldBehavior(worldBehavior3.Name,
                    Converters.convertListToJson(worldBehavior3.FoodCoords));
            }
            catch
            {
                // ignored
